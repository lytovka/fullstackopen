...
const AnecdoteList = ({ anecdotes }) => (
  <div>
    <h2>Anecdotes</h2>
    <ul>
      {anecdotes.map(anecdote => <Link to={`/anecdotes/${anecdote.id}`} key={anecdote.id}><Anecdote anecdote={anecdote} /></Link>)}
    </ul>
  </div>
)
...
const App = () => {
  ...
  const anecdoteById = (id) => {
    const found = anecdotes.find(a => Number(a.id) === Number(id))
    return found
  }
